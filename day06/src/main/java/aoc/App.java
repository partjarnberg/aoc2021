...
import static java.util.Arrays.stream; import static java.util.Collections.rotate; import static java.util.stream.IntStream.rangeClosed;

public class App {
    public long simulate(final List<Long> lanternfishLifecycle, final int numberOfDays) {
        rangeClosed(1, numberOfDays).forEach(day -> {
            rotate(lanternfishLifecycle, -1);
    ...
    public static void main(String[] args) throws IOException {
        final List<Long> lanternfishLifecycle = new ArrayList<>(stream(new long[9]).boxed().toList());
        stream(Files.readString(Path.of("input.txt")).split(",")).map(Integer::parseInt).forEach(stage -> lanternfishLifecycle.set(stage, lanternfishLifecycle.get(stage) + 1 ));
